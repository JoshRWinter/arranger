...
Targa::Targa(const char *filename){
	...
	if(file.gcount()!=1)
		throw std::runtime_error(std::string("file ") + filename + " corrupt");
	compressed=(image_type>>3)&1;
	if(compressed)
		throw std::runtime_error(std::string("file ") + filename + " is compressed");
	...
	if(file.gcount()!=1)
		throw std::runtime_error(std::string("file ") + filename + " corrupt");
	if(bpp!=32&&bpp!=24)
		throw std::runtime_error(std::string("file ") + filename + " has unsupported bpp (" + std::to_string(bpp) + ")");
